...
calc_realistic_truth <- function(A_name = "glp1", nsamp=100000, return_data=FALSE){
  ...
  Ya0_t1 <- mean(d.never$dementia_1,na.rm=T)
  Ya0_t2 <- mean(d.never$dementia_2,na.rm=T)
  Ya0_t3 <- mean(d.never$dementia_3,na.rm=T)
  Ya0_t4 <- mean(d.never$dementia_4,na.rm=T)
  Ya0_t5 <- mean(d.never$dementia_5,na.rm=T)
  Ya0_t6 <- mean(d.never$dementia_6,na.rm=T)
  Ya0_t7 <- mean(d.never$dementia_7,na.rm=T)
  Ya0_t8 <- mean(d.never$dementia_8,na.rm=T)
  Ya0_t9 <- mean(d.never$dementia_9,na.rm=T)
...
average_truth <- function(truth){
  truth <- truth %>% group_by(time) %>%
    summarise(meanYa1=mean(Ya1), meanYa0=mean(Ya0), meanRR=mean(RR), meanRD=mean(RD),
